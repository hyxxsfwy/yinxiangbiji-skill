...
本次正式任务使用 `templates/keyword-union-export-job.json`，日期区间为 `[2026-01-01, 2026-04-01)`，即包含 2026-01-01、不包含 2026-04-01。模板保留用户输入的规范词 `HugginFace`，并额外搜索 `HuggingFace` 和 `Hugging Face`；报告仍归并到规范词。七个主题目录中的正式名称是 `两性情感`，不得继续创建 `婚姻情感` 目录。禁止把中文 JSON 通过 PowerShell 管道或 `python -` 传递，必须从 UTF-8 文件加载：
...
第一次正式文件写入前，脚本为任务声明的七个目录创建 `snapshots/<job-id>-before.zip` 和 SHA-256 清单。无覆盖迁移要求：旧目录 `婚姻情感` 迁移前必须先快照；若同路径目标存在且内容不同，立即中止迁移，不覆盖、删除或替换任一方内容；仅 SHA-256 完全一致时才可去除旧副本。重建索引并完成验收后，才可以删除空目录。完成门禁同时要求所有查询项 `pulled == total`、每个候选都有当前 `keyword_analyses` 记录、候选恒等式成立、关键词 frontmatter 与选择指纹一致、附件和索引完整、日期无越界、领域内及跨领域重复为零，最终 JSON 报告为 `ok: true`。
