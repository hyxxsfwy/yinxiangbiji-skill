...
固定受管十二领域为：AI、Quant、信息技术、投资理财、知识管理、健康医学、中医、两性情感、个人成长、科技产业、自然科学、文史社政。每个领域的索引只收录 `30_精选资料/<domain>/YYYY年MM月/*.md` 中 frontmatter 为 `type: 资料` 且 `domain` 与当前领域完全匹配的文档。

领域契约升级只自动执行注册表声明的旧领域改名，包括“软件工程”到“信息技术”和“历史与社会”到“文史社政”的目录、frontmatter 和精确链接迁移。其余既有资料由 `audit` 生成建议，未经人工 decisions 不自动移动或移入废纸篓。
