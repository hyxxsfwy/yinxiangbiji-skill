...
首次物化前会在 `.state/yinxiang-notes/snapshots/` 中生成 `<job-id>-before.zip` 与 SHA-256 清单。无覆盖迁移要求：旧目录 `婚姻情感` 迁移前必须先快照；若同路径目标存在且内容不同，立即中止迁移，不覆盖、删除或替换任一方内容；仅 SHA-256 完全一致时才可去除旧副本。关键词任务发现非规范旧副本时会移入可恢复隔离区；同一来源的不同内容版本使用内容哈希后缀分别保存，隔离清单不会丢弃旧版本。重新物化已有 Markdown 会保留其中的 `llmwiki:auto-links` 受控双向链接区块。重建索引并完成验收后，才可以删除空目录。最终报告只有在所有查询项 `pulled == total`、候选全部进入 `keyword_analyses`、候选总数恒等式成立、frontmatter 指纹一致、附件和索引完整、日期无越界且重复项为零时才会得到 `ok: true`。

关键词并集导出验收成功后，会自动只保留当前任务的一组完整导出快照，并删除其他 16 位任务 ID 对应的旧 ZIP 与清单。重分类、迁移等其他工作流快照以及孤立或无法安全识别的文件不会被删除。报告字段 `snapshot_cleanup` 给出实际删除文件、释放字节、保留任务和跳过项目；限流、验收失败或当前快照校验失败时不执行自动清理。
