...
首次物化前会在 `.state/yinxiang-notes/snapshots/` 中生成 `<job-id>-before.zip` 与 SHA-256 清单。无覆盖迁移要求：旧目录 `婚姻情感` 迁移前必须先快照；若同路径目标存在且内容不同，立即中止迁移，不覆盖、删除或替换任一方内容；仅 SHA-256 完全一致时才可去除旧副本。重建索引并完成验收后，才可以删除空目录。最终报告只有在所有查询项 `pulled == total`、候选全部进入 `keyword_analyses`、候选总数恒等式成立、frontmatter 指纹一致、附件和索引完整、日期无越界且重复项为零时才会得到 `ok: true`。
