...
首次物化前会在 `.state/yinxiang-notes/snapshots/` 中生成 `<job-id>-before.zip` 与 SHA-256 清单。迁移旧目录 `婚姻情感` 时必须先快照，再执行无覆盖迁移；重建索引并完成验收后，才可以删除空目录。最终报告只有在所有查询项 `pulled == total`、候选全部进入 `keyword_analyses`、候选总数恒等式成立、frontmatter 指纹一致、附件和索引完整、日期无越界且重复项为零时才会得到 `ok: true`。
